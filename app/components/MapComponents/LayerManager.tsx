...
export default function LayerManager({ members = [], onClose }: LayerManagerProps) {
  ...
  const checkbox = (key: string, label: string, checked: boolean, onChange: () => void, color?: string) => (
    <label 
      key={key}
      style={{ display: "flex", alignItems: "center", gap: 12, margin: "8px 0", cursor: "pointer" }}
      onClick={(e) => {
        e.preventDefault();
        onChange();
      }}
    >
      ...
      <span style={{ color: color || "black", fontSize: 14, fontFamily: "Manrope", fontWeight: 400 }}>{label}</span>
    </label>
  );
  ...
  const dropdownHeader = (title: string, isOpen: boolean, setOpen: (open: boolean) => void) => {
    return (
      <div 
        style={{ 
      ...
      >
        ...
        <div style={{ 
          ...
          {checkbox(
            ...
            () => setPOIFilter({ status: toggleArrayValue(filter.poiFilter.status, PoiAcceptanceStatus.ACCEPTED) }),
            "#79A44D"
          )}
          ...
          {checkbox(
            ...
            () => setPOIFilter({ status: toggleArrayValue(filter.poiFilter.status, PoiAcceptanceStatus.PENDING) }),
            "#FF9E44"
          )}
          ...
          {checkbox(
            ...
            () => setPOIFilter({ status: toggleArrayValue(filter.poiFilter.status, PoiAcceptanceStatus.DECLINED) }),
            "#E6393B"
          )}
            ...
            <div style={{ 
              ...
              {Object.values(PoiCategory).map((cat) =>
                checkbox(
                  ...
                  cat === "FOOD" ? "Food" :
                  cat === "ACCOMODATION" ? "Accommodation" : "Other",
                  filter.poiFilter.category.includes(cat),
                  () => setPOIFilter({ category: toggleArrayValue(filter.poiFilter.category, cat) })
                )
              )}
            </div>
          )}
            ...
            <div style={{ 
              ...
              {members.map((member) =>
                checkbox(
                  ...
                  () => setPOIFilter({ 
                    creatorUserIds: toggleArrayValue(filter.poiFilter.creatorUserIds, member.userId) 
                  })
                )
              )}
            </div>
            ...
            <div style={{ 
              ...
              {Object.values(PoiPriority).map((pri) =>
                checkbox(
                  ...
                  () => setPOIFilter({ priority: toggleArrayValue(filter.poiFilter.priority, pri) }),
