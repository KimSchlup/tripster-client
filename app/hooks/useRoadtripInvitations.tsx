...
// Create context
const RoadtripInvitationContext = createContext<RoadtripInvitationContextType | undefined>(undefined);
...
export const RoadtripInvitationProvider: React.FC<RoadtripInvitationProviderProps> = ({
  children,
  pollingInterval = 5000, // Default to 5 seconds
}) => {
  const [pendingInvitations, setPendingInvitations] = useState<Roadtrip[]>([]);
  const [seenInvitationIds, setSeenInvitationIds] = useState<Set<number>>(new Set());
      ...
      if (Array.isArray(data)) {
        ...
        const pendingInvites = data.filter(
          (roadtrip) => {
            ...
            if (typeof roadtrip.invitationStatus === 'string' && 
        ...
        const newInvitations = pendingInvites.filter(
          (invite) => invite.roadtripId && !seenInvitationIds.has(invite.roadtripId)
        );
...
export const useRoadtripInvitations = (): RoadtripInvitationContextType => {
  ...
  if (context === undefined) {
    throw new Error("useRoadtripInvitations must be used within a RoadtripInvitationProvider");
  }
  return context;
};
